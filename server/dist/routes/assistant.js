...
router.post('/query', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const accountsSummary = context.accounts.map(a => `${a.name} (${a.type}${a.subtype ? ` - ${a.subtype}` : ''}): ${a.currency} ${a.balance.toFixed(2)}`).join('\n');
        const topCategoriesSummary = context.topCategories.map(c => `${c.category}: ${currencySymbol}${c.amount.toFixed(2)}`).join('\n');
        const recentTransactionsSummary = context.recentTransactions.slice(0, 10).map(t => `${t.date.toISOString().split('T')[0]}: ${t.name} - ${currencySymbol}${Math.abs(t.amount).toFixed(2)} (${t.category || 'Uncategorized'})`).join('\n');
