...
router.get('/advanced', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const goals = await Goal_1.default.find({ userId, status: 'active' });
        ...
        // ===== TREND INSIGHTS =====
        const trendInsights = await generateTrendInsights(last30Transactions, last60Transactions, last90Transactions, last365Transactions, userCurrency);
        // ===== PREDICTIVE ANALYSIS =====
        const predictions = await generatePredictions(allTransactions, accounts, goals, userCurrency);
        // ===== RECOMMENDATIONS =====
        const recommendations = await generateRecommendations(userId.toString(), accounts, allTransactions, goals, userCurrency);
        // ===== ANOMALY DETECTION =====
...
async function generateTrendInsights(last30, last60, last90, last365, currency) {
    ...
    const previousMonthExpenses = last60
        .filter(t => {
        if (t.date < new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) && t.amount < 0)
            return true;
        const name = (t.name || '').toLowerCase();
...
async function generatePredictions(transactions, accounts, goals, currency) {
...
async function generateRecommendations(userId, accounts, transactions, goals, currency) {
...
function detectAnomalies(transactions, currency) {
    const anomalies = [];
    const last30 = transactions.filter(t => t.date >= new Date(Date.now() - 30 * 24 * 60 * 60 * 1000));
    // Detect duplicates
    const txnMap = {};
...
function deriveCategory(t) {
    ...
    if (t.merchantName) {
        ...
        if (merchant.includes('amazon') || merchant.includes('shop'))
            return 'Shopping';
        if (merchant.includes('gas'))
            return 'Gas';
        return 'General';
    }
    if (t.name) {
...
function generateGoalTip(goal, currency) {
    const progress = (goal.currentAmount / goal.targetAmount) * 100;
    const remaining = goal.targetAmount - goal.currentAmount;
    if (progress >= 100) {
...
//# sourceMappingURL=insights-advanced.js.map