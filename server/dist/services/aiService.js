...
class AIService {
    ...
    async getFinancialContext(userId) {
        // Get all accounts
        const accounts = await Account_1.default.find({ userId }).lean();
...
`;
     ...
     */
    async generatePersonalizedResponse(userId, userMessage, conversationHistory = []) {
        try {
...
Provide a helpful, PERSONALIZED response using their ACTUAL financial data:`;
     ...
     */
    async generateGenericResponse(userMessage) {
        try {
...
`;
     ...
     */
    async generateSuggestedQuestions(userId) {
        try {
            const context = await this.getFinancialContext(userId);
            const suggestions = [];
